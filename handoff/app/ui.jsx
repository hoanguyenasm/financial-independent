/* ============================================================
   FIRE Tracker — shared UI primitives  (exports to window)
   ============================================================ */
...
function Money({ base, cur, amount, inline, signed, size = 'inherit', weight = 800, className = '' }) {
  const showOrig = cur && cur !== 'USD';
  const baseStr = FMT.usd(base, Number.isInteger(base) ? 0 : 2);
    ...
    <div style={{ position: 'relative' }}>
      ...
      {hov != null && (
        <div style={{ position: 'absolute', top: 0, left: `${(pad + hov * bw + bw / 2) / w * 100}%`, transform: 'translateX(-50%)', background: 'var(--surface-3)', border: '1px solid var(--border-2)', borderRadius: 10, padding: '8px 11px', pointerEvents: 'none', whiteSpace: 'nowrap', boxShadow: '0 8px 24px rgba(0,0,0,.5)' }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--pos)' }}>+{FMT.compactUSD(data[hov].income)} in</div>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--neg)' }}>−{FMT.compactUSD(data[hov].expense)} out</div>
          <div style={{ fontSize: 12, fontWeight: 700, marginTop: 2 }}>Net {FMT.compactUSD(data[hov].net)}</div>
        </div>
      )}
    </div>
  );
